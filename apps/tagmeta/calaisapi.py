...
Requires json, which is available from:
...
"""
    ...
    def analyze(self, text, content_type='text/html',
        ...
        try:
            ...
            if hasattr(e, 'reason'):
                assert False, ("calaisapi.py failed to reach the Calais server.", e.reason)
            elif hasattr(e, 'code'):
                assert False, ("The server couldn't fulfill the request.", e.code)
            return {}
        except Exception, e:
    ...
    def construct_json_response(self, data):
        import json
        intermediate_json = json.loads(data)
