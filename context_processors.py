    ...
    if top5s is None:
        ...
        top5s = {
            ...
            'trending': sorted( itertools.chain( 
                            Application.objects.extra(
                            select={ 'hit_count': 'SELECT COUNT(*) AS recent_hits FROM hitcount_hit_count AS t INNER JOIN hitcount_hit AS h ON h.hitcount_id = t.id WHERE h.created > (NOW() - INTERVAL 3 DAY) AND t.content_type_id=' + 
